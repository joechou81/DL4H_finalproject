...
RANDOM_SEED = 12345           # tune weight initialization
...
class StageNet(nn.Module):
    ...
    def step(self, inputs, c_last, h_last, interval):
        ...
        x_out = x_out1 + x_out2
        f_master_gate = self.cumax(x_out[:, :self.levels], 'l2r')              # left to right
        f_master_gate = f_master_gate.unsqueeze(2)
        i_master_gate = self.cumax(x_out[:, self.levels:self.levels*2], 'r2l') # right to left
...
class StageNet_reduce1(nn.Module):
    def __init__(self, input_dim, hidden_dim, conv_size, output_dim, levels, dropconnect=0., dropout=0., dropres=0.3):
        super(StageNet_reduce1, self).__init__()
    ...
    def step(self, inputs, c_last, h_last, interval):
        ...
        c_in = torch.tanh(x_out[:, self.levels*3:])
        c_last = c_last.reshape(-1, self.levels, self.chunk_size)
        c_out = f_gate * c_last + i_gate * c_in  # normal LSTM
        h_out = o_gate * torch.tanh(c_out)
        c_out = c_out.reshape(-1, self.hidden_dim)
        h_out = h_out.reshape(-1, self.hidden_dim)
        out = torch.cat([h_out, f_master_gate[..., 0], i_master_gate[..., 0]], 1)
    ...
    def forward(self, input, time, device):
        ...
        for t in range(time_step):
            ...
            #without Re-weighted convolution operation
            local_dis = tmp_dis.permute(1, 0)
            local_dis = torch.cumsum(local_dis, dim=1)
            local_dis = torch.softmax(local_dis, dim=1)
            local_h = tmp_h.permute(1, 2, 0)
...
class StageNet_reduce2(nn.Module):
    def __init__(self, input_dim, hidden_dim, conv_size, output_dim, levels, dropconnect=0., dropout=0., dropres=0.3):
        super(StageNet_reduce2, self).__init__()
...
class LSTM(nn.Module):
    ...
    def forward(self, input, time, device):
        ...
        origin_h = []
        for t in range(time_step):
            c_out, h_out = self.step(input[:, t, :], c_out, h_out, time[:, t])
            origin_h.append(h_out)
...
class tLSTM(nn.Module):
    ...
    def step(self, inputs, c_last, h_last, interval):
        ...
        o_gate = torch.sigmoid(x_out[:, self.levels*2:self.levels*3])
        c_in = torch.tanh(x_out[:, self.levels*3:])
        c_last_discount = torch.tanh(c_last) *  (1 / (torch.log(1 + interval))) # include time-decay mechanism
        c_last = c_last.reshape(-1, self.levels, self.chunk_size)
        c_last_discount = c_last_discount.reshape(-1, self.levels, self.chunk_size)
        c_last_long = c_last - torch.tanh(c_last)
        c_adj = c_last_discount + c_last_long
        c_out = f_gate * c_adj + i_gate * c_in
        h_out = o_gate * torch.tanh(c_out)
        c_out = c_out.reshape(-1, self.hidden_dim)
        h_out = h_out.reshape(-1, self.hidden_dim)
        return c_out, h_out
...
class StageNet_reduce3(nn.Module):
    def __init__(self, input_dim, hidden_dim, conv_size, output_dim, levels, dropconnect=0., dropout=0., dropres=0.3):
        super(StageNet_reduce3, self).__init__()
        print("StageNet_reduce3\n")
        assert hidden_dim % levels == 0
        self.dropout = dropout
        self.dropconnect = dropconnect
        self.dropres = dropres
...
class ON_LSTM(nn.Module):
    ...
    def cumsoftmax(self, x): # similar approach from StageNet, but different formula
        x = torch.softmax(x, dim=-1)
        x = torch.cumsum(x, dim=-1)
        return x
    ...
    def step(self, inputs, c_last, h_last, interval):
        ...
        if self.dropconnect:
            ...
            x_out2 = self.nn_dropconnect_r(x_out2)
        x_out = x_out1 + x_out2
        f_master_gate = self.cumsoftmax(x_out[:, :self.levels])
        f_master_gate = f_master_gate.unsqueeze(2)
        i_master_gate = 1 - self.cumsoftmax(x_out[:, self.levels:self.levels*2])
        i_master_gate = i_master_gate.unsqueeze(2)
        x_out = x_out[:, self.levels*2:]
        x_out = x_out.reshape(-1, self.levels*4, self.chunk_size)
        f_gate = torch.sigmoid(x_out[:, :self.levels])
        ...
        c_last = c_last.reshape(-1, self.levels, self.chunk_size)
        overlap = f_master_gate * i_master_gate
        c_out = overlap * (f_gate * c_last + i_gate * c_in) + (f_master_gate - overlap) * c_last + (i_master_gate - overlap) * c_in  # cell gate for ON-LSTM
        h_out = o_gate * torch.tanh(c_out)
        c_out = c_out.reshape(-1, self.hidden_dim)
        h_out = h_out.reshape(-1, self.hidden_dim)
        out = torch.cat([h_out, f_master_gate[..., 0], i_master_gate[..., 0]], 1)
